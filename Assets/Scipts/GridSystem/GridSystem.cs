...
namespace Grid
{
    ...
    public class GridSystem
    {
            ...
            get
            {
                ...
                if (gridSystem == null)
                {
                    if (File.Exists(Application.dataPath + "/RTS_data/Map.json"))
                    {
                        ...
                        gridSystem.fromJson(jsonText);
                        if (Initialized == false) Debug.LogError("GridSystem not initialized properly");
                    }
                    else
                    {
                        gridSystem = new GridSystem();
        ...
        {
            if (x >= 0 && z >= 0 && x + width <= current.width && z + height <= current.height) return true;
            else return false;
        }
        ...
        public Vector2Int getBlankGrid(Vector2Int gridCoordinate, int width, int height)
        {
            ...
            for (int i = 0; i < width; i++)
            {
                if (gridCoordinate.x + i >= 0 && gridCoordinate.x + i < current.width && gridCoordinate.y >= 0 && !gridDataArray[gridCoordinate.x + i, gridCoordinate.y].IsOccupied)
                {
            ...
            for (int j = 0; j < height; j++)
            {
                if (gridCoordinate.y + j >= 0 && gridCoordinate.y + j < current.height && gridCoordinate.x >= 0 && !gridDataArray[gridCoordinate.x, gridCoordinate.y + j].IsOccupied)
                {
            ...
            for (int i = 0; i < width; i++)
            {
                if (gridCoordinate.x + i >= 0 && gridCoordinate.x + i < current.width && gridCoordinate.y + height - 1 < current.height && !gridDataArray[gridCoordinate.x + i, gridCoordinate.y + height - 1].IsOccupied)
                {
            ...
            for (int j = 0; j < height; j++)
            {
                if (gridCoordinate.y + j >= 0 && gridCoordinate.y + j < current.height && gridCoordinate.x + width - 1 < current.width && !gridDataArray[gridCoordinate.x + width - 1, gridCoordinate.y + j].IsOccupied)
                {
